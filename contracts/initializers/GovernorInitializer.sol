...
contract GovernorInitializer {
    ...
    * @param pluginData Encoded parameters to create a new token.
     */
    function setup(address governorInstance, bytes calldata pluginData) public payable returns (address instance){
        (
        ...
        bytes32 salt = sha256(pluginData);
        ISeriesToken newToken = ISeriesToken(Clones.cloneDeterministic(addresses[1], salt));
        IOtoCoGovernor newGovernor = IOtoCoGovernor(Clones.cloneDeterministic(governorInstance, salt));
