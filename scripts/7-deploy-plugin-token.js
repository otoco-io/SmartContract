...
async function main() {
    ...
    console.log(`${Bright}🚀 OtoCo Token Deployed Address:${Reset}${FgMagenta}${token.address}${Reset}`);
    console.log(`${Bright}🚀 Token plugin Deployed Address:${Reset}${FgMagenta}${tokenPlugin.address}${Reset}`);
    ...
    if (network.config.chainId != '31337') {
        ...
        await hre.run("verification", {
            addr: tokenPlugin.address,
            args: JSON.stringify([deploysJson.master, token.address, [], []]),
        });
    }
}
