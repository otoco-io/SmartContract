...
describe("OtoCo MultisigV2 Plugin Test", function () {
  ...
  it("Deploy plugin, add and remove", async function () {
    ...
    const addressBeforeRemove = await multisigPlugin.multisigDeployed(0, 1);
    ...
    await expect(transaction).to.emit(multisigPlugin, 'MultisigRemoved').withArgs(0, addressBeforeRemove);
    
    // After removing index 1, should have 2 multisigs left
    expect(await multisigPlugin.multisigPerEntity(0)).to.be.equals(2);
